...
def fgraph_from_model(model: Model) -> FunctionGraph:
    ...
    other_named_vars = [
        var.copy(var.name) for var in model.named_vars.values() if var not in accounted_for
    ]
    value_vars = [val for val in rvs_to_values.values() if val not in other_named_vars]
    ...
    for _ in range(len(value_vars)):
        fgraph.remove_output(-1)
